...
/**
 * @brief  Default drive curve. Modifies the input with an exponential curve. If
 * the input is 127, the function will always output 127, no matter the value of
 * scale, likewise for -127. A scale of zero disable the curve entirely. This
 * curve was inspired by team 5225, the Pilons. A Desmos graph of this curve can
 * be found here: https://www.desmos.com/calculator/rcfjjg83zx
 ...
 * @param curveGain the scale inputted into the drive curve function. If you are
 * using the default drive curve, refer to the `defaultDriveCurve`
 * documentation.
 */
...
/**
 * @brief Control the robot during the driver using the curvature drive control
 * scheme. This control scheme is very similar to arcade drive, except the
 * second joystick axis controls the radius of the curve that the drivetrain
 * makes, rather than the speed. This means that the driver can accelerate in a
 * turn without changing the radius of that turn. This control scheme defaults
 * to arcade when forward is zero.
 * @param throttle speed to move forward or backward. Takes an input from -127
 * to 127.
 * @param turn speed to turn. Takes an input from -127 to 127.
 * @param curveGain the scale inputted into the drive curve function. If you are
