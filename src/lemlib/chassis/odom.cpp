...
// global variables
lemlib::OdomSensors odomSensors(nullptr, nullptr, nullptr, nullptr,
                                nullptr); // the sensors to be used for odometry
lemlib::Drivetrain drive(nullptr, nullptr, 0, 0, 0,
                         0);          // the drivetrain to be used for odometry
...
lemlib::Pose lemlib::getSpeed(bool radians) {
  if (radians)
    ...
    return lemlib::Pose(odomSpeed.x, odomSpeed.y, radToDeg(odomSpeed.theta));
}
...
lemlib::Pose lemlib::getLocalSpeed(bool radians) {
  if (radians)
    ...
    return lemlib::Pose(odomLocalSpeed.x, odomLocalSpeed.y,
...
void lemlib::update() {
  ...
  if (odomSensors.vertical1 != nullptr)
    vertical1Raw = odomSensors.vertical1->getDistanceTraveled();
  if (odomSensors.vertical2 != nullptr)
    vertical2Raw = odomSensors.vertical2->getDistanceTraveled();
  if (odomSensors.horizontal1 != nullptr)
    horizontal1Raw = odomSensors.horizontal1->getDistanceTraveled();
  if (odomSensors.horizontal2 != nullptr)
    horizontal2Raw = odomSensors.horizontal2->getDistanceTraveled();
  ...
  // else, if both vertical tracking wheels aren't substituted by the
  // drivetrain, use the vertical tracking wheels
  else if (!odomSensors.vertical1->getType() &&
           !odomSensors.vertical2->getType())
    heading += (deltaVertical1 - deltaVertical2) /
               (odomSensors.vertical1->getOffset() -
                odomSensors.vertical2->getOffset());
  ...
  if (!odomSensors.vertical1->getType())
    verticalWheel = odomSensors.vertical1;
  else if (!odomSensors.vertical2->getType())
  ...
  if (verticalWheel != nullptr)
    rawVertical = verticalWheel->getDistanceTraveled();
  ...
  if (deltaHeading == 0) { // prevent divide by 0
    ...
    localX =
        2 * sin(deltaHeading / 2) * (deltaX / deltaHeading + horizontalOffset);
    localY =
        2 * sin(deltaHeading / 2) * (deltaY / deltaHeading + verticalOffset);
  }
  ...
  odomSpeed.theta =
      ema((odomPose.theta - prevPose.theta) / 0.01, odomSpeed.theta, 0.95);
