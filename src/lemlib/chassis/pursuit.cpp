 ...
 */
float circleIntersect(lemlib::Pose p1, lemlib::Pose p2, lemlib::Pose pose,
                      float lookaheadDist) {
  // calculations
 ...
 */
lemlib::Pose lookaheadPoint(lemlib::Pose lastLookahead, lemlib::Pose pose,
 ...
 */
float findLookaheadCurvature(lemlib::Pose pose, float heading,
                             lemlib::Pose lookahead) {
  // calculate whether the robot is on the left or right side of the circle
  float side = lemlib::sgn(std::sin(heading) * (lookahead.x - pose.x) -
 ...
 * default
 */
void lemlib::Chassis::follow(const asset &path, float lookahead, int timeout,
