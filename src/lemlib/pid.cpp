 ...
 */
lemlib::FAPID::FAPID(float kF, float kA, float kP, float kI, float kD,
                     std::string name) {
  this->kF = kF;
  this->kA = kA;
  this->kP = kP;
 ...
 * @param maxTime
 */
void lemlib::FAPID::setExit(float largeError, float smallError, int largeTime,
...
float lemlib::FAPID::update(float target, float position, bool log) {
  ...
  if (kA != 0)
    output = lemlib::slew(output, prevOutput, kA);
  prevOutput = output;
  prevError = error;
  totalError += error;
